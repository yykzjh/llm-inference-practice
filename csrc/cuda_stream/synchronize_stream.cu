...
void launch_in_thread(int thread_id) {
    ...
    // initialize blocking stream
    cudaStream_t blocking_stream;
    cudaStreamCreateWithFlags(&blocking_stream, cudaStreamDefault);

    // synchronize legacy default stream
    printf("Synchronize legacy default stream\n");
    ...
    printf("Synchronize legacy default stream completed\n");
    cudaDeviceSynchronize();

    // synchronize blocking stream
    printf("Synchronize blocking stream\n");
    ...
    cudaStreamSynchronize(blocking_stream);
    printf("Synchronize blocking stream completed\n");
    cudaDeviceSynchronize();
